...
export function AppointmentsChart() {
  ...
  const appointmentsData = useMemo(() => {
    ...
    const weekCount = getScheduledInPeriod(weekStart, weekEnd);
    
    // This month
    const monthStart = startOfMonth(today);
    const monthEnd = endOfMonth(today);
    const monthCount = getScheduledInPeriod(monthStart, monthEnd);
    
    // This year
    const yearStart = startOfYear(today);
    const yearEnd = endOfYear(today);
    const yearCount = getScheduledInPeriod(yearStart, yearEnd);
  ...
  const activeData = appointmentsData[activePeriod];
    ...
    <Card className="p-6 bg-card border border-border shadow-sm">
      ...
      <div className="p-4 rounded-lg bg-muted/50 border border-border/50 mb-6">
        ...
        <p className="font-sans text-sm text-muted-foreground mt-1">
          Consultas - {periodLabels[activePeriod]}
        </p>
      </div>
      ...
      <div className="h-48 bg-muted/30 rounded-lg p-4 border border-border/50">
        <ChartContainer config={chartConfig} className="h-full w-full">
          <AreaChart data={chartData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
            <defs>
              <linearGradient id="appointmentsChartGradient" x1="0" y1="0" x2="0" y2="1">
            ...
            <Area
              ...
              strokeWidth={2}
              fill="url(#appointmentsChartGradient)"
            />
          </AreaChart>
        </ChartContainer>
      </div>
