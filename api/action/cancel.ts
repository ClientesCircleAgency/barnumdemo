/**
 * GET /api/action/cancel?token=xxx
 * Public action link for patients to cancel appointments via WhatsApp link
 */
...
export default async function handler(
    ...
    try {
        ...
        // Verify action type
        if (validation.action_type !== 'cancel') {
            return res.status(400).json({
                success: false,
        ...
        const { error: updateError } = await supabaseAdmin
            ...
            .update({
                status: 'cancelled',
                notes: 'Cancelled by patient via WhatsApp link',
        ...
        // Note: The trigger will automatically create a reschedule_patient_cancel workflow
      ...
      <body>
        <div class="container">
          ...
          <div class="info-box">
            <p>💬 Em breve você receberá uma mensagem no WhatsApp com opções para reagendar.</p>
          </div>
